...
'''
y_train = pd.DataFrame([var.tolist() for var in y_train]).fillna(0)
'''
...
#%% Rolling Window
'''
rolloff_mean = df_main[data_columns].rolling(7, center=True).mean()
...
df_stats = result = pd.concat([rolloff_mean, rolloff_median, rolloff_std], axis=1, join="inner").dropna()
rolloff_mean.head(10)
x_data = df_stats[["X","Y","Z"]]
y_data = df_stats.index.get_level_values("Act")
'''
'''
fig, ax = plt.subplots()
...
for var in x_data:
    for var2 in acts:
        autocorrelation_lag7 = df_main[var].loc[df_main[["Act"] == {acts}]].autocorr(lag=7)
        print("7 period Lag for Act = {acts} and acc = {var}: ", autocorrelation_lag7)
        autocorrelation_lag20 = df_main[var].loc[df_main[acts] == '{acts}'].autocorr(lag=20)
        print("20 period Lag for Act = {acts} and acc = {var} ", autocorrelation_lag20)
        autocorrelation_lag50 = df_main[var].loc[df_main[acts] == '{acts}'].autocorr(lag=50)
        print("50 period Lag for Act = {acts} and acc = {var}: ", autocorrelation_lag50)
        autocorrelation_lag100 = df_main[var].loc[df_main[acts] == '{acts}'].autocorr(lag=100)
        print("100 period Lag for Act = {acts} and acc = {var}: ", autocorrelation_lag100)
'''
...
'''
def Own_model():
    df_main.set_index(['Timestamp', 'Act'], inplace= True)
...
random_state = 0
test_size = 0.3
classif_model = Own_model() 
X_train, X_test, y_train, y_test = train_test_split(classif_model, y_data, test_size=test_size, random_state=random_state)
classif_model.fit(X_train, y_train)
y_pred = classif_model.predict(X_test)
something= f1_score(y_test, y_pred, average='micro')
print(something)
